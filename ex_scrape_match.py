...
def match_articles_with_events(articles_data, events_data):
    ...
    for article in articles_data:
        ...
        for event in events_data:
            event_date = normalize_date(event['datetime'])

            # Kräv att datum matchar exakt (inklusive år)
            if article_date.date() != event_date.date():
                continue
            ...
            # Striktare matchning: minst 3 gemensamma ord OCH minst ett av orden måste vara unikt viktigt
            if len(common_words) >= 3 and contains_significant_word(common_words):
    ...
    print(
        f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - {new_matches_count} nya matchningar hittade. {len(matched_results)} totalt sparade.")


def contains_significant_word(words):
    # Lista över viktiga ord som ger högre relevans
    significant_words = {
        'mord', 'död', 'skjut', 'kniv', 'rån', 'brand', 'explosion',
        'olycka', 'krock', 'misshandel', 'spräng', 'hot', 'våld',
        'fängelse', 'gripen', 'anhalt', 'brott', 'straff'
    }
    return any(word in significant_words for word in words)
